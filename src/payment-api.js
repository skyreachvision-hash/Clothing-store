...
import { getEmailSettings, sendTransactionalEmail } from "./email-service.js";
...
async function sendOrderConfirmationEmail(env, transaction, order, checkoutData) {
  const settingsRow = await env.DB.prepare("SELECT settings_json FROM store_settings WHERE id = 1").first();
  const emailSettings = getEmailSettings(settingsRow);
  if (!emailSettings.enabled || !emailSettings.notify_order_confirmation) return;
  ...
  const shippingText = [customer.address, customer.city, customer.province, customer.postal_code, customer.country].filter(Boolean).map(escapeHtml).join(", ");
  ...
  const html = "<div style=\"font-family:Arial,sans-serif;line-height:1.6;color:#222\">" +
    ...
    "<tr><td style=\"padding:12px 0 4px\"><strong>Subtotal</strong></td><td style=\"padding:12px 0 4px;text-align:right\"><strong>" + escapeHtml(transaction.currency) + " " + Number(checkoutData?.line_items?.reduce((sum, item) => sum + Number(item.unit_price || 0) * Number(item.quantity || 0), 0) || 0).toFixed(2) + "</strong></td></tr>" +
  ...
  const text = "Order confirmed\n\n" +
    ...
    "Items:\n" + textItems + "\n\n" +
    "Subtotal: " + clean(transaction.currency) + " " + Number(lineItems.reduce((sum, item) => sum + Number(item.unit_price || 0) * Number(item.quantity || 0), 0)).toFixed(2) + "\n" +
    ...
    (shippingText ? "Delivery address: " + shippingText.replace(/<[^>]*>/g, "") + "\n\n" : "") +
...
async function handleVerify(request, env) {
  ...
  if (transaction.provider === "yoco") {
    ...
    if (status === "completed" && amountMatches && currencyMatches) {
      ...
      if (order?.created) {
        await sendOrderConfirmationEmail(env, transaction, order, JSON.parse(transaction.checkout_data || "{}"));
      }
