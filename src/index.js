...
function readDerElement(bytes, offset) {
  ...
  if (length & 0x80) {
    ...
    if (!lengthBytes || lengthBytes > 4 || offset + 2 + lengthBytes > bytes.length) throw new Error("Invalid certificate length.");
    length = 0;
...
async function handleStoreSettings(request, env) {
  ...
  try {
    ...
    for (const social of socialLinks) {
      ...
      if (existing?.id) {
        await env.DB.prepare(`UPDATE social_links SET label = ?, url = ?, sort_order = ?, is_enabled = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).bind(label, url, sortOrder, enabled, existing.id).run();
      } else {
        await env.DB.prepare(`INSERT INTO social_links (platform, label, url, sort_order, is_enabled) VALUES (?, ?, ?, ?, ?)`).bind(platform, label, url, sortOrder, enabled).run();
      }
    }
...
async function handleImageUpload(request, env) {
  ...
  try {
    ...
    if (contentLength > 10 * 1024 * 1024) return jsonResponse({ success: false, error: "Image is too large. Maximum size is 10 MB." }, 413);
    ...
    const folder = "clothing-store/products";
    const signature = await createCloudinarySignature({ folder, timestamp }, env.CLOUDINARY_API_SECRET);
...
async function getCategories(env, includeDisabled = false) {
  const query = includeDisabled ? `SELECT id, name, slug, description, sort_order, is_enabled, created_at, updated_at FROM categories ORDER BY sort_order ASC, id ASC` : `SELECT id, name, slug, description, sort_order, is_enabled, created_at, updated_at FROM categories WHERE is_enabled = 1 ORDER BY sort_order ASC, id ASC`;
...
async function handleCategories(request, env) {
  if (request.method === "GET") {
    try {
      ...
      if (includeDisabled) { try { await verifyFirebaseIdToken(request); } catch { return jsonResponse({ success: false, error: "Authentication required." }, 401); } }
      return jsonResponse({ success: true, data: await getCategories(env, includeDisabled) });
    } catch { return jsonResponse({ success: false, error: "Unable to load categories." }, 500); }
  }
  try {
    const token = await verifyFirebaseIdToken(request);
    ...
    if (request.method === "POST") {
      ...
      const result = await env.DB.prepare(`INSERT INTO categories (name, slug, description, sort_order, is_enabled) VALUES (?, ?, ?, ?, ?) RETURNING id`).bind(name, slug, String(body?.description ?? "").trim(), parseNonNegativeInteger(body?.sort_order), parseEnabled(body?.is_enabled)).first();
...
async function getProducts(env, request) {
  ...
  let sql = `SELECT p.id, p.category_id, p.name, p.slug, p.description, p.sku, p.price, p.currency, p.status, p.stock_quantity, p.track_stock, p.sort_order, p.created_at, p.updated_at, c.name AS category_name, c.slug AS category_slug FROM products p LEFT JOIN categories c ON c.id = p.category_id WHERE 1 = 1`;
  const bindings = [];
  if (publicOnly) sql += ` AND p.status = 'active'`;
  else if (!["draft", "active", "archived"].includes(status)) return { error: "Invalid product status.", status: 400 };
  else { sql += ` AND p.status = ?`; bindings.push(status); }
  ...
  if (products.length) {
    ...
    for (const image of imagesResult.results ?? []) { if (!imagesByProduct.has(image.product_id)) imagesByProduct.set(image.product_id, []); imagesByProduct.get(image.product_id).push(image); }
...
async function handleProducts(request, env) {
  if (request.method === "GET") {
    try { const result = await getProducts(env, request); if (result.error) return jsonResponse({ success: false, error: result.error }, result.status); return jsonResponse({ success: true, data: result.data }); }
    catch { return jsonResponse({ success: false, error: "Unable to load products." }, 500); }
  }
  try {
    const token = await verifyFirebaseIdToken(request);
    if (!["POST", "PUT", "DELETE"].includes(request.method)) return jsonResponse({ success: false, error: "Method not allowed." }, 405);
    ...
    if (categoryId) { const category = await env.DB.prepare(`SELECT id FROM categories WHERE id = ?`).bind(categoryId).first(); if (!category) return jsonResponse({ success: false, error: "Category not found." }, 400); }
    ...
    let productId = id;
    if (request.method === "POST") {
      const result = await env.DB.prepare(`INSERT INTO products (category_id, name, slug, description, sku, price, currency, status, stock_quantity, track_stock, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id`).bind(...productValues).first();
      productId = result.id;
    } else {
      await env.DB.prepare(`UPDATE products SET category_id = ?, name = ?, slug = ?, description = ?, sku = ?, price = ?, currency = ?, status = ?, stock_quantity = ?, track_stock = ?, sort_order = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).bind(...productValues, productId).run();
    }
    if (Array.isArray(body?.images)) {
      ...
      for (const image of body.images) {
        ...
        await env.DB.prepare(`INSERT INTO product_images (product_id, image_url, cloudinary_public_id, alt_text, sort_order, is_primary) VALUES (?, ?, ?, ?, ?, ?)`).bind(productId, imageUrl, String(image?.cloudinary_public_id ?? "").trim(), String(image?.alt_text ?? "").trim(), parseNonNegativeInteger(image?.sort_order), parseEnabled(image?.is_primary)).run();
      }
    }
    if (Array.isArray(body?.variants)) {
      ...
      for (const variant of body.variants) {
        ...
        await env.DB.prepare(`INSERT INTO product_variants (product_id, name, sku, option_values, price_override, stock_quantity, is_enabled, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).bind(productId, String(variant?.name ?? "").trim(), String(variant?.sku ?? "").trim(), JSON.stringify(optionValues), priceOverride, parseNonNegativeInteger(variant?.stock_quantity), parseEnabled(variant?.is_enabled), parseNonNegativeInteger(variant?.sort_order)).run();
      }
    }
    return jsonResponse({ success: true, data: { id: productId, uid: token.sub } }, request.method === "POST" ? 201 : 200);
  } catch (error) {
    if (error?.message === "Authentication required.") return jsonResponse({ success: false, error: error.message }, 401);
    if (String(error?.message || "").includes("UNIQUE constraint failed")) return jsonResponse({ success: false, error: "A product with that slug already exists." }, 409);
