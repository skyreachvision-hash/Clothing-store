...
async function getOrCreateProductGroup(env, name) { const normalizedName = String(name ?? "").trim(); if (!normalizedName) return null; const slug = normalizeSlug(normalizedName); if (!slug) return null; const existing = await env.DB.prepare(`SELECT id FROM product_groups WHERE slug = ?`).bind(slug).first(); if (existing) return existing.id; const created = await env.DB.prepare(`INSERT INTO product_groups (name, slug, relationship_type) VALUES (?, ?, 'color') RETURNING id`).bind(normalizedName, slug).first(); return created?.id ?? null; }
async function getProductGroups(env) { const result = await env.DB.prepare(`SELECT id, name, slug, relationship_type, created_at, updated_at FROM product_groups ORDER BY name COLLATE NOCASE ASC, id ASC`).all(); return result.results ?? []; }
async function handleProductGroups(request, env) {
  if (request.method === "GET") { try { const token = await verifyFirebaseIdToken(request); await requireAdminRole(token, env); return jsonResponse({ success: true, data: await getProductGroups(env) }); } catch (error) { if (error?.message === "Administrator authorization required.") return jsonResponse({ success: false, error: error.message }, 403); return jsonResponse({ success: false, error: "Authentication required." }, 401); } }
  try {
    const token = await verifyFirebaseIdToken(request); await requireAdminRole(token, env);
    const body = await request.json();
    if (request.method === "POST") {
      const name = String(body?.name ?? "").trim(); const relationshipType = String(body?.relationship_type ?? "color").trim().toLowerCase();
      if (!name) return jsonResponse({ success: false, error: "Group name is required." }, 400);
      if (!["color", "size"].includes(relationshipType)) return jsonResponse({ success: false, error: "Invalid relationship type." }, 400);
      const slug = normalizeSlug(name); if (!slug) return jsonResponse({ success: false, error: "Group name is invalid." }, 400);
      const result = await env.DB.prepare(`INSERT INTO product_groups (name, slug, relationship_type) VALUES (?, ?, ?) RETURNING id`).bind(name, slug, relationshipType).first();
      return jsonResponse({ success: true, data: { id: result.id } }, 201);
    }
    if (request.method === "PUT") {
      const id = parseOptionalId(new URL(request.url).searchParams.get("id")); if (!id) return jsonResponse({ success: false, error: "A valid group id is required." }, 400);
      const name = String(body?.name ?? "").trim(); const relationshipType = String(body?.relationship_type ?? "color").trim().toLowerCase();
      if (!name || !["color", "size"].includes(relationshipType)) return jsonResponse({ success: false, error: "Valid group name and relationship type are required." }, 400);
      const slug = normalizeSlug(name); await env.DB.prepare(`UPDATE product_groups SET name = ?, slug = ?, relationship_type = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).bind(name, slug, relationshipType, id).run();
      return jsonResponse({ success: true, data: { id } });
    }
    if (request.method === "DELETE") {
      const id = parseOptionalId(new URL(request.url).searchParams.get("id")); if (!id) return jsonResponse({ success: false, error: "A valid group id is required." }, 400);
      const assigned = await env.DB.prepare(`SELECT COUNT(*) AS total FROM products WHERE product_group_id = ?`).bind(id).first();
      if (Number(assigned?.total || 0) > 0) return jsonResponse({ success: false, error: "This group is still assigned to products. Remove the assignments first." }, 409);
      await env.DB.prepare(`DELETE FROM product_groups WHERE id = ?`).bind(id).run(); return jsonResponse({ success: true, data: { id } });
    }
    return jsonResponse({ success: false, error: "Method not allowed." }, 405);
  } catch (error) {
    if (error?.message === "Administrator authorization required.") return jsonResponse({ success: false, error: error.message }, 403);
    if (String(error?.message || "").includes("UNIQUE constraint failed")) return jsonResponse({ success: false, error: "A product group with that name already exists." }, 409);
    return jsonResponse({ success: false, error: "Unable to save product group." }, 500);
  }
}
...
async function getProducts(env, request) {
  const params = new URL(request.url).searchParams; const status = params.get("status"); const productId = parseOptionalId(params.get("id")); const categoryId = parseOptionalId(params.get("category_id")); const search = String(params.get("search") || "").trim(); const page = Math.max(1, parseNonNegativeInteger(params.get("page"), 1)); const limit = Math.min(100, Math.max(1, parseNonNegativeInteger(params.get("limit"), 24))); const offset = (page - 1) * limit; const publicOnly = status === null;
  let sql = `SELECT p.id, p.category_id, p.product_group_id, p.product_group_value, p.product_color, p.product_size, p.name, p.slug, p.description, p.sku, p.price, p.currency, p.status, p.stock_quantity, p.track_stock, p.sort_order, p.created_at, p.updated_at, c.name AS category_name, c.slug AS category_slug, pg.name AS product_group_name, pg.slug AS product_group_slug, pg.relationship_type AS product_group_relationship_type FROM products p LEFT JOIN categories c ON c.id = p.category_id LEFT JOIN product_groups pg ON pg.id = p.product_group_id WHERE 1 = 1`; const bindings = [];
  ...
  if (productId) { sql += ` AND p.id = ?`; bindings.push(productId); }
  ...
  if (products.length) { const ids = products.map((product) => product.id); const placeholders = ids.map(() => "?").join(", "); const [imagesResult, variantsResult] = await Promise.all([env.DB.prepare(`SELECT id, product_id, image_url, cloudinary_public_id, alt_text, sort_order, is_primary, created_at FROM product_images WHERE product_id IN (${placeholders}) ORDER BY sort_order ASC, id ASC`).bind(...ids).all(), env.DB.prepare(`SELECT id, product_id, name, sku, option_values, price_override, stock_quantity, is_enabled, sort_order, created_at, updated_at FROM product_variants WHERE product_id IN (${placeholders}) ORDER BY sort_order ASC, id ASC`).bind(...ids).all()]); const imagesByProduct = new Map(); const variantsByProduct = new Map(); for (const image of imagesResult.results ?? []) { if (!imagesByProduct.has(image.product_id)) imagesByProduct.set(image.product_id, []); imagesByProduct.get(image.product_id).push(image); } for (const variant of variantsResult.results ?? []) { let optionValues = {}; try { optionValues = JSON.parse(variant.option_values || "{}"); } catch { optionValues = {}; } const normalizedVariant = { ...variant, option_values: optionValues }; if (!variantsByProduct.has(variant.product_id)) variantsByProduct.set(variant.product_id, []); variantsByProduct.get(variant.product_id).push(normalizedVariant); } for (const product of products) { product.images = imagesByProduct.get(product.id) ?? []; product.variants = variantsByProduct.get(product.id) ?? []; product.related_products = []; } if (publicOnly) { const groupIds = [...new Set(products.map((product) => product.product_group_id).filter(Boolean))]; if (groupIds.length) { const groupPlaceholders = groupIds.map(() => "?").join(", "); const relatedResult = await env.DB.prepare(`SELECT p.id, p.product_group_id, p.product_group_value, p.name, p.price, p.currency, p.stock_quantity, p.track_stock, p.status FROM products p WHERE p.status = 'active' AND p.product_group_id IN (${groupPlaceholders}) ORDER BY p.sort_order ASC, p.id DESC`).bind(...groupIds).all(); const related = relatedResult.results ?? []; if (related.length) { const relatedIds = related.map((item) => item.id); const relatedImagePlaceholders = relatedIds.map(() => "?").join(", "); const relatedImages = await env.DB.prepare(`SELECT id, product_id, image_url, alt_text, sort_order, is_primary FROM product_images WHERE product_id IN (${relatedImagePlaceholders}) ORDER BY sort_order ASC, id ASC`).bind(...relatedIds).all(); const relatedImagesByProduct = new Map(); for (const image of relatedImages.results ?? []) { if (!relatedImagesByProduct.has(image.product_id)) relatedImagesByProduct.set(image.product_id, []); relatedImagesByProduct.get(image.product_id).push(image); } const relatedByGroup = new Map(); for (const item of related) { item.images = relatedImagesByProduct.get(item.id) ?? []; if (!relatedByGroup.has(item.product_group_id)) relatedByGroup.set(item.product_group_id, []); relatedByGroup.get(item.product_group_id).push(item); } for (const product of products) { product.related_products = (relatedByGroup.get(product.product_group_id) ?? []).filter((item) => item.id !== product.id); } } } } }
...
async function handleProducts(request, env) {
  ...
  try {
    ...
    if (request.method === "PUT") { if (!id) return jsonResponse({ success: false, error: "A valid product id is required." }, 400); existing = await env.DB.prepare(`SELECT id, category_id, product_group_id, product_group_value, name, slug, description, sku, price, currency, status, stock_quantity, track_stock, sort_order FROM products WHERE id = ?`).bind(id).first(); if (!existing) return jsonResponse({ success: false, error: "Product not found." }, 404); }
    ...
    let productGroupId = existing?.product_group_id ?? null;
    if (Object.prototype.hasOwnProperty.call(body ?? {}, "product_group_id")) productGroupId = body.product_group_id === null || body.product_group_id === "" ? null : parseOptionalId(body.product_group_id);
    else if (Object.prototype.hasOwnProperty.call(body ?? {}, "product_group_name")) productGroupId = await getOrCreateProductGroup(env, body.product_group_name);
    let productGroupValue = Object.prototype.hasOwnProperty.call(body ?? {}, "product_group_value") ? String(body.product_group_value ?? "").trim() : String(existing?.product_group_value ?? "").trim();
    if (productGroupId) { const group = await env.DB.prepare(`SELECT id, relationship_type FROM product_groups WHERE id = ?`).bind(productGroupId).first(); if (!group) return jsonResponse({ success: false, error: "Product group not found." }, 400); if (!productGroupValue) return jsonResponse({ success: false, error: "A group value such as a color or size is required when a product group is selected." }, 400); }
    else productGroupValue = "";
    const productColor = String(body?.product_color ?? existing?.product_color ?? "").trim(); const productSize = String(body?.product_size ?? existing?.product_size ?? "").trim();
    const productValues = [categoryId, productGroupId, productGroupValue, productColor, productSize, name, slug, String(body?.description ?? existing?.description ?? "").trim(), String(body?.sku ?? existing?.sku ?? "").trim(), parseNonNegativeNumber(body?.price, existing?.price ?? 0), String(body?.currency ?? existing?.currency ?? "ZAR").trim().toUpperCase() || "ZAR", status, parseNonNegativeInteger(body?.stock_quantity, existing?.stock_quantity ?? 0), parseEnabled(body?.track_stock, existing?.track_stock ?? 1), parseNonNegativeInteger(body?.sort_order, existing?.sort_order ?? 0)];
    let productId = id;
    if (request.method === "POST") { const result = await env.DB.prepare(`INSERT INTO products (category_id, product_group_id, product_group_value, product_color, product_size, name, slug, description, sku, price, currency, status, stock_quantity, track_stock, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id`).bind(...productValues).first(); productId = result.id; }
    else await env.DB.prepare(`UPDATE products SET category_id = ?, product_group_id = ?, product_group_value = ?, product_color = ?, product_size = ?, name = ?, slug = ?, description = ?, sku = ?, price = ?, currency = ?, status = ?, stock_quantity = ?, track_stock = ?, sort_order = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).bind(...productValues, productId).run();
