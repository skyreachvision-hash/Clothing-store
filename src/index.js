...
async function getProducts(env, request) {
  ...
  let sql = `SELECT p.id, p.category_id, p.product_group_id, p.name, p.slug, p.description, p.sku, p.price, p.currency, p.status, p.stock_quantity, p.track_stock, p.sort_order, p.created_at, p.updated_at, c.name AS category_name, c.slug AS category_slug, pg.name AS product_group_name, pg.slug AS product_group_slug FROM products p LEFT JOIN categories c ON c.id = p.category_id LEFT JOIN product_groups pg ON pg.id = p.product_group_id WHERE 1 = 1`; const bindings = [];
...
async function handleProducts(request, env) {
  ...
  try {
    ...
    let productGroupId = existing?.product_group_id ?? null; if (Object.prototype.hasOwnProperty.call(body ?? {}, "product_group_name")) productGroupId = await getOrCreateProductGroup(env, body.product_group_name); const productValues = [categoryId, productGroupId, name, slug, String(body?.description ?? existing?.description ?? "").trim(), String(body?.sku ?? existing?.sku ?? "").trim(), parseNonNegativeNumber(body?.price, existing?.price ?? 0), String(body?.currency ?? existing?.currency ?? "ZAR").trim().toUpperCase() || "ZAR", status, parseNonNegativeInteger(body?.stock_quantity, existing?.stock_quantity ?? 0), parseEnabled(body?.track_stock, existing?.track_stock ?? 1), parseNonNegativeInteger(body?.sort_order, existing?.sort_order ?? 0)];
    let productId = id;
    if (request.method === "POST") { const result = await env.DB.prepare(`INSERT INTO products (category_id, product_group_id, name, slug, description, sku, price, currency, status, stock_quantity, track_stock, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id`).bind(...productValues).first(); productId = result.id; }
    else await env.DB.prepare(`UPDATE products SET category_id = ?, product_group_id = ?, name = ?, slug = ?, description = ?, sku = ?, price = ?, currency = ?, status = ?, stock_quantity = ?, track_stock = ?, sort_order = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).bind(...productValues, productId).run();
